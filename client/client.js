...
function applyBubbleTexture(id) {
  ...
  bubbleDisposer = styles.insert(
    "[data-message-role='user'], [class*='message'] [class*='user'], [data-chat-flow] [class*='user'] { background-image:url('" + t.data + "') !important; background-size:32px 32px !important; background-repeat:repeat !important; image-rendering:pixelated; background-color:" + t.color + " !important; border: 3px solid #000 !important; border-radius: 12px !important; box-shadow: inset 0 2px 0 rgba(200,150,255,0.2), inset 2px 0 0 rgba(200,150,255,0.08), inset 0 -3px 0 rgba(0,0,0,0.6), inset -2px 0 0 rgba(0,0,0,0.4), 0 4px 12px rgba(0,0,0,0.5) !important; color: #fff !important; }" +
    "[data-message-role='user'] *, [class*='message'] [class*='user'] *, [data-chat-flow] [class*='user'] * { color: #fff !important; }" +
    "[data-message-role='user'] *:not(button):not(a), [class*='message'] [class*='user'] *:not(button):not(a), [data-chat-flow] [class*='user'] *:not(button):not(a) { background: transparent !important; border: none !important; box-shadow: none !important; }"
  );
}
...
<<<<<<< HEAD
        React.createElement('div', { style: { marginBottom: 12, color: '#f2f2f2', fontSize: 13 } },
          ...
          React.createElement('div', { style: { display: 'flex', flexWrap: 'wrap', gap: 6 } },
            Object.keys(TEX).map((id) => React.createElement('button', {
              key: id,
              className: 'mc-btn',
              'data-active': snap.bubble === id ? '1' : '0',
              onClick: () => { setStore({ bubble: id }); applyBubbleTexture(id); },
              style: { display: 'flex', alignItems: 'center', gap: 4, padding: '2px 6px', fontSize: 11 },
            },
              React.createElement('span', { style: Object.assign({ width: 16, height: 16, border: '1px solid #000', flexShrink: 0 }, swatchData(TEX[id].data, TEX[id].color)) }),
              TEX[id].name
            ))
          )
        ),
